    ...
    try:
        ...
        for idx, (row_num, row_data) in enumerate(matches, 1):
            ...
            if pending_entry_start_idx is not None:
                ...
                if there_was_gap:
                    ...
                    for g_from, g_to in gap_slots:
                        ...
                        g_diff = round(g_dc_num - g_mw_ramp, 2) if g_dc_num is not None and g_mw_ramp is not None else None
                        ...
                        output_rows.append({
                            ...
                            "Diff (MW)": g_diff if g_diff is not None else "",
                ...
                if entry_end_idx > pending_entry_start_idx:
                    ...
                    output_rows.append({
                        ...
                        "Diff (MW)": "", "Mus": "", "Sum Mus": mus_sum_rounded,
                    })
                    row_idx += 1
            ...
            for slot_idx, (slot_from, slot_to) in enumerate(slots):
                ...
                # Diff (MW) = DC - MW as per ramp (per reference; can go negative when MW > DC)
                diff_value = round(dc_num - mw_as_per_ramp, 2) if dc_num is not None and mw_as_per_ramp is not None else None
...
if _status == "running" and _bg_job and (not _viewing_saved_report or _viewing_generating_report):
    ...
    if _partial_file and _partial_file.exists():
        ...
        if not _partial_rows and _viewing_generating_report:
            ...
            for _col in ("DC (MW)", "As per SLDC Scada in MW", "MW as per ramp", "Diff (MW)", "Mus", "Sum Mus"):
...
if 'display_output_data_key' in st.session_state and not _showing_bg_job_table:
    ...
    if output_data_key in st.session_state:
        ...
        if df_output is not None and not df_output.empty:
            ...
            for col in ("DC (MW)", "As per SLDC Scada in MW", "MW as per ramp", "Diff (MW)", "Mus", "Sum Mus"):
