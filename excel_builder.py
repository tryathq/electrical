...
HEADERS = ["Date", "From", "To", "DC (MW)", "As per SLDC Scada in MW", "MW as per ramp", "Diff (MW)", "Mus", "Sum Mus"]
COLUMN_WIDTHS = [15, 10, 10, 12, 25, 14, 12, 12, 12]
PAD = 0
...
def build_report_workbook(output_rows: list[dict]) -> Workbook:
    ...
    for r, row_dict in enumerate(output_rows):
        ...
        sheet.cell(row=out_row, column=start_col + 5).value = row_dict.get("MW as per ramp")
        sheet.cell(row=out_row, column=start_col + 6).value = row_dict.get("Diff (MW)")
